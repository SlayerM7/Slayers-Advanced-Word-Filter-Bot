module.exports = (client, message, args, db, command) => {
  ...
  if (db.has(`filter_${message.guild.id}_settings.ignore-admins`)) {
    if (
      db.get(`filter_${message.guild.id}_settings.ignore-admins`) === true &&
  ...
  if (db.has(`filter_${message.guild.id}_settings-ignore-roles`)) {
    ...
    rolesArray.map((role) => {
      if (message.member.roles.cache.has(role)) hasOkRole = true;
    });
    if (hasOkRole === true) return;
  }
  if (db.has(`filter_${message.guild.id}_settings-ignore-channels`)) {
    let channelsArray = db.get(
      `filter_${message.guild.id}_settings-ignore-channels`
    );

    if (channelsArray.includes(message.channel.id)) return;
  }
  if (db.has(`filter_${message.guild.id}_settings.ignore-nsfw`)) {
    if (
      db.get(`filter_${message.guild.id}_settings.ignore-nsfw`) === true &&
    ...
    if (args.includes(word)) {
      message
        ...
        .then(() => {
          ...
          let curData = db.get(
            `save_logs_${message.guild.id}.${message.author.id}.num`
          );
          if (curData) {
            db.set(
              `save_logs_${message.guild.id}.${message.author.id}.num`,
              curData.num ? curData.num + 1 : 1
            );
          } else {
